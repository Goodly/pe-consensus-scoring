...
def evaluateCoding(answers, users, starts, ends, numUsers, length, sourceText, hlUsers, hlAns, repDF=None, dfunc=None,
    ...
    if dfunc == 'ordinal':
        ...
        if check_threshold(highScore, numUsers, func = threshold_func) != 'H' and \
...
def passToUnitizing(answers, hlusers, starts, ends, numUsers, length, \
    ...
    if check_threshold(highScore, numUsers, func=threshold_func) == 'H':
        # f for filtered
        ...
        status = check_threshold(highScore, numUsers, func=threshold_func)
