...
#<function you’re testing>_<kind of data in this folder>_<description of test>
#PYTHONPATH=.. pytest
def test_santiy_check(config):
    test_path = test_utils.make_test_directory(config, 'iaa_data_hunt_sanity_test')
    dh = datahunt(out_folder=test_path, source_task_id='owedqdqdq98uqdqa')
    dh.add_row({'answer_label': 'T1.Q1.A1', 'namespace': 'Covid_ArgumentRelevance_2020_03_20v2', 'contributor_uuid': 'Michael'})
    dh.add_row({'answer_label': 'T1.Q1.A1', 'namespace': 'Covid_ArgumentRelevance_2020_03_20v2', 'contributor_uuid': 'Dwight'})
    dh.add_row({'answer_label': 'T1.Q1.A2', 'namespace': 'Covid_ArgumentRelevance_2020_03_20v2', 'contributor_uuid': 'Ricky'})
    fin_path = dh.export()
    data_path = config['data_dir']
    schema_path = data_path + '/schemas'
    new_test_path = test_utils.make_test_directory(config, 'out_iaa_data_hunt_sanity_test')
    iaa_out = calc_agreement_directory(test_path, schema_path, config['IAA_config_dir'], test_utils.texts_dir,
                                       outDirectory=new_test_path)
    print(iaa_out)
    for root, dir, files in os.walk(iaa_out):
        for file in files:coding_perc_agreement
            # should be only 1 file for this case, so just run it on the only one
            # if there's more than 1 then you can get fancy
            out_df = pd.read_csv(os.path.join(iaa_out, file), encoding='utf-8')
    # 9 answer choices to a checklist question
    #agreement score is >2/3
    #make sure there is only 1 row
    #make sure the asnwwers are correct

    assert out_df.loc(out_df['coding_perc_agreement'] > .6)
