    ...
    for q_lab in np.unique(schema['question_label']):
        for j in range(1,6):
            ...
            if has_hl == 0:
                ...
                my_start = start
                my_end = end
            params = {'answer_label': q_lab + '.A' + str(ans), 'namespace': namespace,
...
def test_master(config):
    ...
    for i in range(9):
        for j in range(7):
            dh.add_row({'answer_label': 'T1.Q1.A' + str(j), 'namespace': 'Covid_Languagev1.1',
                            'contributor_uuid': 'User' + str(i), 'start_pos': 19*j, 'end_pos': 20*j+10})
    make_dh(dh, 'Covid_Languagev1.1', 1, 20, start_shift=lambda x: x + 5, end_shift=lambda x: x + 5,
